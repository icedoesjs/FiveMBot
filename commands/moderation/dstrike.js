...
module.exports = {
    ...
    run: async(client, message, args, base, prefix) => {
        if (!message.member.hasPermission("MANAGE_ROLES")) return message.reply(`You don't have permission.`).then((w) => {
            setTimeout(() => {
                w.delete()
            }, 5000)
        })
        ...
        if (message.mentions) {
            ...
            message.reply(`Please type the **reason** for the strike, you have 60 seconds.`).then((w) => {
                message.channel.awaitMessages(filter, {
                    ...
                    embed.setColor('RED')
                    embed.setAuthor(`${user.tag} was department striked!`)
                    embed.setDescription(`Striked by **${message.author.tag}** for **${reason}**`)
                    if (total == undefined || total == null || !total || total == 0) {
                        let total = 1
                        set(`PRP.strikes.dept.${message.guild.id}.${user.id}`, 1)
                        embed.setFooter(`Total department strikes: ${total}`)
                    }
                    if (total == 1) {
                        let total = 2;
                        set(`PRP.strikes.dept.${message.guild.id}.${user.id}`, 2)
                        embed.setFooter(`Total department strikes: ${total}`)
                    }
                    if (total == 2) {
                        let total = 3;
                        set(`PRP.strikes.dept.${message.guild.id}.${user.id}`, 0)
                        embed.setFooter(`Total department strikes: ${total}`)
                    }
